...
public:
    ...
    int length() const { return static_cast<int>(get(CAP_PROP_FRAME_COUNT)); }
};
...
public:
    ...
    StereoDepth(
        const cv::Ptr<cv::StereoSGBM> &block_matcher,
        float fx,
        float baseline
    ) : block_matcher(block_matcher), fx(fx), baseline(baseline) {}

    void to_depth(const cv::Mat &img_left, const cv::Mat &img_right, cv::Mat &depth) const {
        // Compute disparity
        cv::Mat disparity;
